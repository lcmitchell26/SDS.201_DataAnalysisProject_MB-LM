...
To exactly quantify the relationship between the average budget of a movie and whether it passes or fails the Bechdel test, we fit a categorical regression model. In this model, the outcome variable is the budget of a movie and the explanatory variable is the movie passing or failing the aforementioned test. The model’s predictions are visualized in Figure \_\_\_\_ while its predictions are shown in Table \_\_\_\_\_. 
...
Through summarizing the data, we have gathered that the average budget of movies that fail the Alison Bechdel test tend to be higher than the average budgets of movies that pass it, when you adjust for inflation in 2013 dollars. 

-talk about it in the wider range of filmmaking and sexism in the industry stretching beyond recently, discussions have only happened now. - talk about more insights with a more specific look at how other variables may be affected; do certain genres suffer from failing the test more than others? - talk about how it could be for many reasons that we, the researchers are still unsure about, like it could be that writers make deliberate decisions to give importance to main male leads rather than female leads, sexism within writing scripts, sexism from writers and/or executives that help make decisions on budgets. But it is a problem if films without big budgets behind them have actual 3-dimensional characters. Overall
